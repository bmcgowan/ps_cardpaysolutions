...
class Cardpaysolutions extends PaymentModule
{
    ...
    {
      if (isset($_POST['CARDPAYSOLUTIONS_ACCOUNT_USERNAME']) &&
        isset($_POST['CARDPAYSOLUTIONS_ACCOUNT_PASSWORD']) &&
        !empty($_POST['CARDPAYSOLUTIONS_ACCOUNT_USERNAME']) &&
        !empty($_POST['CARDPAYSOLUTIONS_ACCOUNT_PASSWORD']))
      {
  ...
  public function hookDisplayAdminOrderTabOrder($params)
  {
    ...
    $avs_messages = $this->_avsResponses();
    $cvv_messages = $this->_cvvResponses();
    ...
    if ($params['objOrder'] && Validate::isLoadedObject($params['objOrder']) && isset($params['objOrder']->valid))
    {
      if (version_compare(_PS_VERSION_, '1.5', '>=') && isset($params['objOrder']->reference))
        ...
        $this->context->smarty->assign('cardpay_order', array('id' => $params['objOrder']->id, 'valid' => $params['objOrder']->valid));
  ...
  public function validation()
  {
    ...
    if (Validate::isLoadedObject($cart) && !Order::getOrderByCartId((int)Tools::getValue('cart')))
    {
      ...
      $params = array(
        ...
        'address1' => Tools::safeOutput($billingAddress->address1),
        'zip' => Tools::safeOutput($billingAddress->postcode)
      );
  ...
  private function _avsResponses()
  {
    return array(
  ...
  private function _cvvResponses()
  {
    return array(
      'M' => 'CVV2/CVC2 match',
